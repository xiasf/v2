...
// 获取数据
function get_json() {
	$.getJSON(get_category_call_json_url + '?' + Math.random(), {shop_id:$("input[name=shop_id]").val()}, function(data, status) {
		if (status)
			callback(data);
		else
...
function save() {
	...
	$.post(category_edit_url, {str:data_base, shop_id:$("input[name=shop_id]").val()}, function(result, textStatus) {
