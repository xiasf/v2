...
class ShopModel extends CommonModel {
    ...
    protected $insertFields = 'shop_name,seller_id,describe,contacts_mobile,contacts_email,contacts_name,business_status,status,logo,address,sort,lng,lat,shop_type,shop_business_type,address_reference,settlement_account,bank_name,bank_user_name,bank_card,is_seller_settlement,store_bitmap';
    protected $updateFields = 'id,shop_name,describe,contacts_mobile,contacts_email,contacts_name,business_status,status,logo,address,sort,lng,lat,shop_type,address_reference,settlement_account,bank_name,bank_user_name,bank_card,is_seller_settlement,store_bitmap';
    ...
    protected $_validate = array(
        ...
        array('shop_business_type', '1,2', '商店经营类型不合法', self::EXISTS_VALIDATE, 'in'),
    ...
    // 检测商户是否存在
    private function check_seller($seller_id) {
        if (!M('seller')->field('id')->find($seller_id)) {
            return false;
        } else {
            return true;
        }
    ...
    // 绑定打印机
    public function bind_printer($shop_id, $printer_id) {
        if (IS_POST) {
            if (!$this->field('id')->find($shop_id)) {
                $this->error = '分店不存在';
                return false;
            ...
            if (!$shop_printer->extend_add(array('shop_id' => $shop_id, 'printer_id' => $printer_id))) {
                $this->error = $shop_printer->error ? : '绑定失败';
                return false;
            }
            return true;
        }
    ...
    // 解绑打印机
    public function unbundling_printer($shop_id, $printer_id) {
        if (IS_POST) {
            if (!$this->field('id')->find($shop_id)) {
                $this->error = '分店不存在';
                return false;
            ...
            $shop_printer = D('ShopPrinter');
            if (!$shop_printer->shop_printer_extend_del(array('shop_id' => $shop_id, 'printer_id' => $printer_id))) {
                $this->error = $shop_printer->error ? : '解绑失败';
                return false;
            }
            return true;
        }
    ...
    public function shop_add() {
        if (IS_POST) {
            ...
            if (2 == I('post.shop_business_type/d')) {
                if (!$this->check_seller(I('post.seller_id/d'))) {
                    $this->error = '商户不合法';
                    return false;
                }
            } else {
                if (I('post.seller_id/d')) {
                    $this->error = '自营店铺由平台管理';
                    return false;
                }
            }
